...
test("captureJobsConfig reads maxConcurrent 6, hardTimeoutMs, and maxBatch from paths.json", () => {
  ...
  assert.match(captureJobsConfig().awwwards500, /awwwards-500\.json$/);
  assert.match(captureJobsConfig().awwwardsPlus1000, /awwwards-plus-1000\.json$/);
  ...
  assert.match(captureJobsConfig().cssdaWotdPlus1000, /cssda-wotd-plus-1000\.json$/);
...
test("public-sector-1000 has 1000 unique https urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("public-sector-1000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  assert.equal(new Set(catalog.entries.map((entry) => entry.id)).size, 1000);
...
test("awwwards-500 has 500 unique https visit urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("awwwards-500");
  assert.equal(catalog.entries.length, 500);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 500);
  assert.equal(new Set(catalog.entries.map((entry) => entry.id)).size, 500);
  ...
  for (const url of urls) {
    assert.match(url, /^https:\/\//);
    assert.equal(url.includes("awwwards.com"), false);
  }
  const priorHosts = new Set(
    [
      ...catalogUrls(loadCaptureCatalog("automotive-oem-50")),
  ...
  assert.ok(urls.some((url) => url.includes("oimachi") || url.includes("cipher") || hosts.size === 500));
});

test("awwwards-plus-1000 has 1000 unique https visit urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("awwwards-plus-1000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  const hosts = new Set(
...
test("awwwards-plus-2000 has 1000 unique https visit urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("awwwards-plus-2000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  const hosts = new Set(
...
test("awwwards-plus-3000 has 1000 unique https visit urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("awwwards-plus-3000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  const hosts = new Set(
...
test("awwwards-plus-4000 has 1000 unique https visit urls and skips prior catalogs", () => {
  const catalog = loadCaptureCatalog("awwwards-plus-4000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  const hosts = new Set(
...
test("cssda-wotd-plus-1000 has 1000 unique https visit urls and skips prior catalogs including cssda-wotd-1000", () => {
  const catalog = loadCaptureCatalog("cssda-wotd-plus-1000");
  assert.equal(catalog.entries.length, 1000);
  const urls = catalogUrls(catalog);
  assert.equal(new Set(urls).size, 1000);
  const hosts = new Set(
  ...
  const priorHosts = new Set(
    [
      ...catalogUrls(loadCaptureCatalog("cssda-wotd-1000")),
      ...catalogUrls(loadCaptureCatalog("automotive-oem-50")),
      ...catalogUrls(loadCaptureCatalog("cross-industry-100")),
      ...catalogUrls(loadCaptureCatalog("engineering-manufacturing-1000")),
      ...catalogUrls(loadCaptureCatalog("insurance-1000")),
