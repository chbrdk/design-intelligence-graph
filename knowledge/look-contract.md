...
- Generate: `POST /api/library/references/generate` and `dig_generate` apply the same contract to `token_hints` + `constraints` (`generation_version` `0.4.0`). Measured values outrank compact reference tokens and LLM `layout_hints`.
- Screen export: `POST /api/library/analyses/:capture_run_id/prompt-pack` (also `POST /references/prompt-pack` with `capture_run_id`). Uses indexed refs when present, otherwise a synthetic screen reference from vision_page + look_contract. Library detail **Copy prompt pack** writes that JSON to the clipboard.
- Page rhythm: [`page-rhythm.md`](page-rhythm.md) — `page_arc` / bands on the same prompt pack and on generate `0.4.0` (outranks card-kit heroes).
