# Library screen gallery (2026-09-22)
...
2. **Graphics** (`#/library/graphics`) — artboard uploads (`graphicViewport`, default `artboard`): campaign / print / social packages. Not mixed into Screens.
3. **Devices** (`#/library/devices`) — tablet + mobile, with a viewport chip (`all` / `tablet` / `mobile` via `devicesQueryParam`).

Library modes use msqdx-ui `MagazineContentsNav` (sticky Contents, same pattern as CHECKION scan/GEO). Items come from `libraryModeNavItems()` in `apps/web/lib/library-hash.ts`. Copy: `paths.libraryCopy.contentsLabel` / `screensLabel` / `graphicsLabel` / `devicesLabel` / `sectionsLabel` (Modules) / `flowsLabel`. Module gallery: [`library-module-gallery.md`](library-module-gallery.md).

Canonical capture viewports stay `mobile` / `tablet` / `desktop` / `artboard`. The gallery split is display-only; the API still returns every viewport.
