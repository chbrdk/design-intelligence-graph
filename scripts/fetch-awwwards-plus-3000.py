#!/usr/bin/env python3
"""Fourth Awwwards wave: /websites/ page 306+ → Visit-site URLs.

Writes knowledge/catalogs/sources/awwwards-websites-plus-3-2026.json
Run after awwwards-plus-2000 (last_page ~305).
"""
...
OUT = ROOT / "knowledge" / "catalogs" / "sources" / "awwwards-websites-plus-3-2026.json"
PRIOR_SOURCES = [
...
EXTRA_MAX_PAGES = 40
DEFAULT_START_PAGE = 306
DEFAULT_TARGET = 1150
DEFAULT_MAX_PAGE = 600
SLEEP_S = 0.45
...
def write_checkpoint(by_host: dict[str, dict], start_page: int, last_page: int) -> None:
    ...
    OUT.write_text(
        json.dumps(
            {
                "id": "awwwards-websites-plus-3-2026",
...
def main() -> None:
    ...
    if len(by_host) < args.target:
        ...
        for list_path in EXTRA_LIST_PATHS:
            if len(by_host) >= args.target:
                break
            empty_streak = 0
            for page in range(1, EXTRA_MAX_PAGES + 1):
                if len(by_host) >= args.target:
                    break
    ...
    if len(by_host) < 1000:
        raise SystemExit(f"need at least 1000 new hosts for plus-3000 catalog, got {len(by_host)}")
