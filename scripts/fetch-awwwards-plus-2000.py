#!/usr/bin/env python3
"""Third Awwwards wave: /websites/ page 235+ → Visit-site URLs.

Writes knowledge/catalogs/sources/awwwards-websites-plus-2-2026.json
Run after awwwards-plus-1000 (last_page ~234).
"""
...
OUT = ROOT / "knowledge" / "catalogs" / "sources" / "awwwards-websites-plus-2-2026.json"
PRIOR_SOURCES = [
...
def write_checkpoint(by_host: dict[str, dict], start_page: int, last_page: int) -> None:
    ...
    OUT.write_text(
        json.dumps(
            {
                "id": "awwwards-websites-plus-2-2026",
...
def main() -> None:
    ...
    if len(by_host) < 1000:
        raise SystemExit(f"need at least 1000 new hosts for plus-2000 catalog, got {len(by_host)}")
