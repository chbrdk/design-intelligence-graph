#!/usr/bin/env python3
"""Build knowledge/catalogs/cssda-wotd-plus-1000.json from the CSSDA plus scrape."""
...
SOURCE = ROOT / "knowledge" / "catalogs" / "sources" / "cssda-wotd-websites-plus-2026.json"
...
def main() -> None:
    ...
    for row in entries:
        ...
        host = host_only(url)
        if not host or blocked(host) or host in prior_hosts or host in seen:
            continue
        seen.add(host)
        out_entries.append(
            {
                "rank": len(out_entries),
                "id": f"cssda-wotd-plus-{len(out_entries)+1}-{host}",
    ...
    out = {
        "id": "cssda-wotd-plus-1000",
        "title": f"CSS Design Awards WOTD plus curated sites (quality slice; up to {args.target})",
        "source": "CSS Design Awards WOTD winners pages (external outbound target URLs), second wave.",
        "year": 2026,
