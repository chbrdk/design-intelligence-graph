#!/usr/bin/env python3
"""Resume Awwwards scrape from /websites/ page 22+ for Visit-site URLs.

Writes knowledge/catalogs/sources/awwwards-websites-plus-2026.json
Run after the first wave (awwwards-websites-2026.json / awwwards-500).
"""
...
OUT = ROOT / "knowledge" / "catalogs" / "sources" / "awwwards-websites-plus-2026.json"
PRIOR_SOURCE = ROOT / "knowledge" / "catalogs" / "sources" / "awwwards-websites-2026.json"
CATALOG_HOST_FILES = [
...
def load_known_hosts() -> set[str]:
    known: set[str] = set()
    for path in [PRIOR_SOURCE] + CATALOG_HOST_FILES:
        if not path.exists():
            continue
...
def write_checkpoint(by_host: dict[str, dict], start_page: int, last_page: int) -> None:
    ...
    OUT.write_text(
        json.dumps(
            {
                "id": "awwwards-websites-plus-2026",
...
def main() -> None:
    ...
    print(
        ...
        f"already_plus={len(by_host)}, need={needed}"
    )
    ...
    for page in range(args.start_page, args.max_page + 1):
        ...
        if len(slug_queue) >= queue_goal:
            break
        url = f"{BASE}{LIST_PATH}" if page == 1 else f"{BASE}{LIST_PATH}?page={page}"
        try:
            html = fetch(url)
    ...
    if len(by_host) < 1000:
        raise SystemExit(f"need at least 1000 new hosts for plus-1000 catalog, got {len(by_host)}")
