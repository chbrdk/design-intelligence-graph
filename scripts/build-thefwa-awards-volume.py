#!/usr/bin/env python3
"""
Build a Capture catalog from The FWA awards volume Source-JSON.

Input:
  knowledge/catalogs/sources/thefwa-awards-volume-2026.json

Output:
  knowledge/catalogs/thefwa-1000.json
"""
...
SOURCE = ROOT / "knowledge" / "catalogs" / "sources" / "thefwa-awards-volume-2026.json"
OUT = ROOT / "knowledge" / "catalogs" / "thefwa-1000.json"
...
def canonicalize_url(url: str) -> str:
    """Prefer https for capture; keep path/query if present."""
    ...
    if not host:
        return url.strip()
    # Drop credentials/fragments; force https.
    path = parsed.path or "/"
...
def main() -> None:
        ...
        out_entries.append(
            {
                ...
                "id": f"thefwa-{len(out_entries)+1}-{host}",
                "name": row.get("name") or host,
                "group": row.get("group") or "The FWA",
    ...
    out = {
        "id": "thefwa-1000",
        "title": f"The FWA awards curated sites (volume; up to {args.target})",
        "source": "The FWA /api/timeline awards stream (external outbound URLs).",
        "year": 2026,
